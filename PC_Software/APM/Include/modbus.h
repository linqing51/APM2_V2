...
#define MODBUS_RS232 0
#define MODBUS_RS485 1
 ...
 * backends.
 */
#define MODBUS_MAX_ADU_LENGTH              513
...
#define EMBBADLRC  (EMBXGTAR + 2)
#define EMBBADDATA (EMBXGTAR + 3)
#define EMBBADEXC  (EMBXGTAR + 4)
#define EMBUNKEXC  (EMBXGTAR + 5)
#define EMBMDATA   (EMBXGTAR + 6)
#define EMBBADSLAVE (EMBXGTAR + 7)
...
MODBUS_API int	modbus_dchar2hex(uint8_t *to,const uint8_t *from);
MODBUS_API int modbus_string2hex(uint8_t *to,const uint8_t *from ,uint8_t nb);
MODBUS_API int modbus_hex2dchar(uint8_t *to, uint8_t from);
...
/* Modbus_Application_Protocol_V1_1b.pdf Chapter 4 Section 1 Page 5
 * RS232 / RS485 ADU = start(1 bytes) + slave (2 byte) + 506 bytes  + LRC (2 bytes) + end(2 bytes) = 513 bytes
 */
#define MODBUS_ASCII_MAX_ADU_LENGTH  513
MODBUS_API int modbus_ascii_Transmit(uint8_t *msg, int msg_length);
