...
int CNiVisionCtl::Camera_GetExposure(UINT nIndex)
{
	UINT nExposure(0);
	IMAQdxError status = IMAQdxErrorSuccess;
	CStringA attributeName = "CameraAttributes::AcquisitionControl::ExposureTimeAbs";
	bool32 readable = false;
...
//[42] = "CameraAttributes::AcquisitionTrigger::ExposureTimeAbs"
BOOL CNiVisionCtl::Camera_SetExposure(UINT nIndex, UINT nExposure)
{
	IMAQdxError status = IMAQdxErrorSuccess;
	bool32 writable = false;
	CStringA attributeName = "CameraAttributes::AcquisitionTrigger::ExposureTimeAbs";
...
BOOL CNiVisionCtl::NewDisplayBinder( CWnd* pOwner,int nCameraId,int nWndId)
{
...
void CNiVisionCtl::ResizeDisplayWnd(UINT nIndex,CRect rc)
{
	Point pos = {rc.TopLeft().x,rc.TopLeft().y};
	imaqMoveWindow(m_nDisplayBinder[nIndex], pos);
	imaqSetWindowSize(m_nDisplayBinder[nIndex], rc.Width(), rc.Height());
}

BOOL CNiVisionCtl::Camera_Display(UINT nIndex, BOOL bShow, Image* image)
{
	int nSuccess(0);
	static int nXY[4][2] = { 1,1 };
	FLOAT fx(1);
	FLOAT fy(1);
	FLOAT rate(1);
	int nWidth(0);
...
//			RotateImage(nIndex, displayImg);
			if (!m_bEnableZoomTool[m_nDisplayBinder[nIndex]])
			{
				int x=1, y=1;
				nSuccess = !nSuccess ? nSuccess : imaqGetWindowSize(m_nDisplayBinder[nIndex], &nWidth, &nHeight);
				nSuccess = !nSuccess ? nSuccess : imaqGetImageSize(displayImg, &x, &y);
				if (x&&y&&(nXY[m_nDisplayBinder[nIndex]][0] ^ x || nXY[m_nDisplayBinder[nIndex]][1]^y)){
					fx = (FLOAT)nWidth / x;
					fy = (FLOAT)nHeight / y;
					rate= fx < fy ? fx : fy;
					nSuccess = !nSuccess ? nSuccess : imaqZoomWindow2(m_nDisplayBinder[nIndex], rate, rate, IMAQ_NO_POINT);
				}
			}
			nSuccess = !nSuccess ? nSuccess : imaqDisplayImage(displayImg, m_nDisplayBinder[nIndex], FALSE);
		} else{
...
void CNiVisionCtl::SetParent(UINT nIndex, CWnd* pWndParent)
{
	if (nIndex < m_nCameraNum&&pWndParent)
		CWnd::FromHandle((HWND)imaqGetSystemWindowHandle(m_nDisplayBinder[nIndex]))->SetParent(pWndParent);
}
...
BOOL CNiVisionCtl::CreateTemplate(Image* dest, const Image* source, Rect rc, Image* mask, float offsetx , float offsety )
{
	...
	RotationAngleRange angle = { 0, 360 };
...
BOOL CNiVisionCtl::CreateBinary(Image* dest, const Image* source, UINT nMode)
{
	int nSuccess(FALSE);
	UINT i(0);
...
BOOL CNiVisionCtl::CreateGeomTemplate(Image* dest, const Image* source, Rect rc, BYTE nContrast, Image* mask, float offsetx, float offsety)
{
	...
	nSuccess = !nSuccess ? nSuccess : imaqLearnGeometricPattern2(dest, IMAQ_NO_OFFSET, 0, &curveOptions, &lgpao, mask);
...
void CNiVisionCtl::DrawLine(Image* dest, Line line, UINT nIndex, const RGBValue*  crText)
{
	int nSuccess(0);
	ROI* roi = imaqCreateROI();
	CStringA gp;
	...
	if (crText)
		defaultcolor = *crText;
	imaqSetROIColor(roi, &defaultcolor);
	nSuccess = imaqAddLineContour(roi, line.start,line.end);
...
BOOL CNiVisionCtl::DisplayLocalImage(UINT nIndex, CString FilePath)
{
	int nSuccess(0);
	if (nIndex >= m_nCameraNum)
		return FALSE;
...
BOOL CNiVisionCtl::CameraCalib(UINT nIndex, Image* source)
{
	if (nIndex >= m_nCameraNum)
		return FALSE;
...
Image* CNiVisionCtl::CreateImage(UINT nWidth, UINT nHeight, ImageType type /*= IMAQ_IMAGE_U8*/)
{
	int nSuccess(0);
	Image* 	TempImg;
	TempImg = imaqCreateImage(IMAQ_IMAGE_U8, 2);
	if (TempImg)
		nSuccess = imaqSetImageSize(TempImg, nWidth, nHeight);
	if (!nSuccess)
	{
		DisplayNIVisionError(imaqGetLastError());
		return NULL;
...
void CNiVisionCtl::EnableZoomDisplay(UINT nIndex, BOOL bEnable)
{
	imaqSetCurrentTool(bEnable ? IMAQ_ZOOM_TOOL : IMAQ_NO_TOOL);
	m_bEnableZoomTool[nIndex] = bEnable;
}
...
FindEdgeReport* CNiVisionCtl::FindEdgeLine(Image *image, EdgeData straightdata, UINT nGp)
{
	...
	straightEdgeOptions.searchMode = IMAQ_USE_BEST_RAKE_EDGES;
	straightEdgeOptions.minScore = 90;
	...
	straightEdgeOptions.minCoverage = 50;
